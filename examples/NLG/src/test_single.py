...
class MultiplyStage(nn.Module):
    def __init__(self,device,config,multiplier):
        ...
        self.ln1 = nn.Linear(1,1).to(device)
        self.ln2 = nn.Linear(1,1).to(device)
        self.ln3 = nn.Linear(1,1).to(device)
        self.ln4 = nn.Linear(1,1).to(device)
    def forward(self, x):
        return self.ln4(self.ln3(self.ln2(self.ln1(x))))
...
if __name__ == '__main__':
    ...
    model = TestModel([0], local_rank, config, 1,pipeline=True)
